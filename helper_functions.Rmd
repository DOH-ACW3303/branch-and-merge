...
# allow mdy or ymd with - or / separator
detect_date <- function(x) {
 
    str_detect(x, "^[:digit:]{2,4}[-,/][:digit:]{1,2}[-,/][:digit:]{1,2}$") |
      str_detect(x, "^[:digit:]{1,2}[-,/][:digit:]{1,2}[-,/][:digit:]{2,4}$")
...
convert_excel_date <- function(x) {
  ...
  # if input matches expected date format, return as data
  if(detect_date(x)) {
    return(x)
  }
  
  else if(str_detect(x, "^[:digit:]{4,5}$")) {
